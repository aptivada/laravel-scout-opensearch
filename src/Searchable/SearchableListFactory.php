...
final class SearchableListFactory
{
    ...
    private function find(): array
    {
        ...
        return array_values(
            array_filter($this->getSearchableClasses(), static function (string $class) use ($appNamespace) {
    ...
    private function getProjectClasses(): Collection
    {
        ...
        $projectClasses = collect();

        $nodeClasses = Collection::make($nodes);

        foreach ($nodeClasses as $nodeClass) {
            if (isset($nodeClass->namespacedName)) {
                $projectClasses->add($nodeClass->namespacedName->toCodeString());
            }
        }
    ...
    private function findSearchableTraitRecursively(string $class): bool
    {
        try {
            ...
            ) && $this->findSearchableTraitRecursively($parent->getName());
